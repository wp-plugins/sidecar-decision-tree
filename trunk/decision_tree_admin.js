    ...
    <div class="decision_tree_row">\
        ...
        <div class="decision_tree_cell decision_tree_cell_label">\
            <p class="description description-wide">\
                    ...
                    name="' + step_number + '_question_text" placeholder="(required)">\
                    </label>\
            </p>\
            <p class="description description-wide decision_tree_input_answers">\
                <label for="step_' + step_number + '_textlink" >Text Link<br />\
                <input type="text" id="' + step_number + '_textlink" size="60" \
                class="decision_tree_input   widefat edit-menu-item-title"\
                name="' + step_number + '_textlink" placeholder="(optional)">\
                </label>\
            </p>\
            <p class="description description-wide">\
                ...
                class="decision_tree_input  widefat edit-menu-item-title"\
                name="' + step_number + '_subtext" placeholder="(optional)">\
                </label>\
            </p>\
            <p class="description description-wide decision_tree_input_answers">\
                <label for="step_' + step_number + '_info" >Info<br />\
                <textarea id="' + step_number + '_info" cols="30" rows="5"\
                class="decision_tree_input   widefat edit-menu-item-title"\
                name="' + step_number + '_info" \
        ...
        <div class="decision_tree_choice_add">\
            <div class="decision_tree_row">\
                <div class="decision_tree_cell">\
                    <input type="button" class="button decision_tree_add_choice decision_tree_choice_area" \n\
...
DTree.prototype.populateDTFormFromDB = function(DTree_object_restored) {
    ...
    jQuery.each(DTree_object_restored.data, function(step_number, questionObj) {
        if (typeof step_number != "undefined" && typeof questionObj != "undefined" ){
            ...
            jQuery('#' + step_number + '_textlink').val(questionObj.textlink);
            jQuery('#' + step_number + '_info').val(questionObj.info);

            if (questionObj.type == 'answer'){
                jQuery('#decision_tree_step-' + step_number + ' .decision_tree_input_answers').show();
                jQuery('#decision_tree_step-' + step_number + ' .decision_tree_choice_area').hide();
            }
        }        
...
DTree.prototype.getDTObjectFromForm = function(getFormIDs) {
    ...
    jQuery.each(steps, function() {
        ...
        DTree_object.data[step_number].subtext = jQuery('#' + step_number +
                '_subtext').val();
        DTree_object.data[step_number].textlink = jQuery('#' + step_number +
                '_textlink').val();
        DTree_object.data[step_number].info = jQuery('#' + step_number +
                '_info').val();
...
DTree.prototype.check_step_type = function(parentID, type){
	...
	if(type == 'answer'){
		...
		jQuery('#' + parentID + ' .decision_tree_input_answers').show();

	} else {
		jQuery('#' + parentID + ' .decision_tree_choice_area').show();
		jQuery('#' + parentID + ' .decision_tree_input_answers').hide();
	}
